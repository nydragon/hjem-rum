{
  ...
  toFish = strOrDeriv: fileName:
    if (typeOf strOrDeriv) == "string"
    then pkgs.writers.writeFish fileName strOrDeriv
    else if isDerivation strOrDeriv
    then strOrDeriv
    else throw "Input is of invalid type ${typeOf strOrDeriv}, expected `path` or `string`.";

  toFishFunc = strOrDeriv: funcName:
    toFish (
      if (typeOf strOrDeriv) == "string"
      then ''
        function ${funcName};
